        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div style="background:linear-gradient(to right,#0c1e3f,#07122e)" class="px-5 py-4">
                    <h3 class="text-sm font-semibold text-white">Edit Mahasiswa</h3>
                </div>
                <div class="p-6">
                    <form method="POST" action="{{ route('mahasiswa.update', $mahasiswa) }}" enctype="multipart/form-data" id="mahasiswaForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="nim" class="block mt-1 w-full" type="text" name="nim" :value="old('nim', $mahasiswa->nim)" required maxlength="20" placeholder="Contoh: 220102001" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="nama" class="block mt-1 w-full" type="text" name="nama" :value="old('nama', $mahasiswa->nama)" required placeholder="Nama lengkap" />
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-4 mt-2">
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" name="jenis_kelamin" value="L" @checked(old('jenis_kelamin', $mahasiswa->jenis_kelamin) == 'L') required class="text-[#0c1e3f] focus:ring-blue-300">
                                            <span class="text-sm text-gray-700">Laki-laki</span>
                                        </label>
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="radio" name="jenis_kelamin" value="P" @checked(old('jenis_kelamin', $mahasiswa->jenis_kelamin) == 'P') required class="text-[#0c1e3f] focus:ring-blue-300">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $mahasiswa->email)" required placeholder="mahasiswa@example.com" />
                                    <x-input-error :messages="$errors->get('email')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="tgl_lahir" :value="__('Tanggal Lahir')" />
                                    <x-text-input id="tgl_lahir" class="block mt-1 w-full" type="date" name="tgl_lahir" :value="old('tgl_lahir', $mahasiswa->tgl_lahir?->format('Y-m-d'))" required />
                                ...
                                <div>
                                    ...
                                    <select id="jurusan" name="jurusan" class="border-gray-300 focus:border-gray-900 focus:ring-blue-300 rounded-xl shadow-sm w-full mt-1" required>
                                        ...
                                        <optgroup label="Fakultas Bisnis dan Keuangan">
                                            <option value="Manajemen" @selected(old('jurusan', $mahasiswa->jurusan) == 'Manajemen')>Manajemen</option>
                                            <option value="Akuntansi" @selected(old('jurusan', $mahasiswa->jurusan) == 'Akuntansi')>Akuntansi</option>
                                            <option value="Pendidikan Matematika" @selected(old('jurusan', $mahasiswa->jurusan) == 'Pendidikan Matematika')>Pendidikan Matematika</option>
                                        </optgroup>
                                        <optgroup label="Fakultas Industri Kreatif">
                                            <option value="Pendidikan Bahasa Inggris" @selected(old('jurusan', $mahasiswa->jurusan) == 'Pendidikan Bahasa Inggris')>Pendidikan Bahasa Inggris</option>
                                            <option value="Sains Komunikasi" @selected(old('jurusan', $mahasiswa->jurusan) == 'Sains Komunikasi')>Sains Komunikasi</option>
                                            <option value="Desain Komunikasi Visual" @selected(old('jurusan', $mahasiswa->jurusan) == 'Desain Komunikasi Visual')>Desain Komunikasi Visual</option>
                                            <option value="Sistem Informasi" @selected(old('jurusan', $mahasiswa->jurusan) == 'Sistem Informasi')>Sistem Informasi</option>
                                            <option value="Ilmu Komputer" @selected(old('jurusan', $mahasiswa->jurusan) == 'Ilmu Komputer')>Ilmu Komputer</option>
                                        </optgroup>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select id="angkatan" name="angkatan" class="border-gray-300 focus:border-gray-900 focus:ring-blue-300 rounded-xl shadow-sm w-full mt-1" required>
                                        ...
                                        @for ($y = date('Y'); $y >= 2021; $y--)
                                            <option value="{{ $y }}" @selected(old('angkatan', $mahasiswa->angkatan) == $y)>{{ $y }}</option>
                                        @endfor
                                    </select>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="alamat" name="alamat" rows="3" placeholder="Alamat lengkap" class="border-gray-300 focus:border-gray-900 focus:ring-blue-300 rounded-xl shadow-sm w-full mt-1">{{ old('alamat', $mahasiswa->alamat) }}</textarea>
                            ...
                            <div>
                                ...
                                <div class="flex items-start gap-4 mt-1">
                                    <div id="currentFotoContainer">
                                        <img id="currentFoto" src="{{ $mahasiswa->foto ? Storage::url($mahasiswa->foto) : asset('images/profile default.jpeg') }}"
                                             class="h-24 w-24 object-cover rounded-xl border border-gray-200 shadow-sm" alt="Foto {{ $mahasiswa->nama }}">
                                    </div>
                                    <div class="flex-1">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-5 border-t border-gray-100">
                            ...
                            <button type="submit" style="background:#0c1e3f" class="inline-flex items-center px-4 py-2 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm" onmouseover="this.style.background='#07122e'" onmouseout="this.style.background='#0c1e3f'">
                                <svg class="w-4 h-4 me-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>
                                Update
                            </button>
                        </div>
                    </form>
    ...
    <div id="cropModal" class="fixed inset-0 z-50 hidden bg-black/70 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
                ...
                <select id="aspectRatio" class="text-sm border border-gray-200 rounded-xl px-3 py-1.5 bg-white/50">
...
document.getElementById('confirmCrop').addEventListener('click', function() {
    ...
    cropper.getCroppedCanvas({ maxWidth: 1024, maxHeight: 1024 }).toBlob(function(blob) {
        ...
        document.getElementById('currentFotoContainer') && document.getElementById('currentFotoContainer').classList.add('hidden');
        document.getElementById('cropModal').classList.add('hidden');
        if (cropper) { cropper.destroy(); cropper = null; }
    }, 'image/jpeg', 0.9);
});
